 ...
 if(value && fileType !== 'pdf') {
    return (
        <div className="relative h-20 w-20">
          ...
          <button onClick={() => onChange('')} className="bg-rose-500 text-neutral-100 p-1 rounded-full absolute
           top-0 right-0 shadow-sm" type="button">
            <X className="h-4 w-4"/>
          </button>
        </div>
    )
 ...
 if(value && fileType === 'pdf') {
  return (
    <div className="relative flex items-center p-2 mt-2 rounded-md bg-background/10 ">
      ...
      <button onClick={() => onChange('')} className="bg-rose-500 text-neutral-100 p-1 rounded-full absolute
           -top-2 -right-2 shadow-sm" type="button">
            <X className="h-4 w-4"/>
       </button>
    </div>
  )
